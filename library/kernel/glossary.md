...
| **Process map** (`_map/`, `MAPL`·`MAPN`·`MAPE`·`MAPD`·`MAPG`) | The published structure of the understood as-is process — lanes, steps, decisions, exceptions, hand-offs, outputs and consumers, and what exists at each block — with evidence references; organises knowledge, never the authority of a fact (`process_map.py`). | o mapa do processo, com o que se sabe de cada passo |
| **Map validation** (`D-NNN — Mapa do processo mp-vNN validado`) | The owner's check of the map's representation and scope for one published version; confirms no hypothesis and closes no doubt. | o dono reconhece o processo como está desenhado, com as dúvidas que ficam |
| **Orphan** (`orphans`) | A unit the capture produced with no place in the map: `gap_in_map` · `undetermined` · `out_of_scope`. Never coverage. | o que ficou fora do mapa, e porquê |
