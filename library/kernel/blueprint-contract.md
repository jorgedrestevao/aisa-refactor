...
- Approval: a `D-NNN` block in `decisions.md` ("Blueprint bp-v<NN> aprovado") + the matching SU row. In a `handoff-v1` engagement the block is generated by `functional.py approval-block` and carries `**Blueprint sha256**` of the approved version: a later change to that file reads `stale`, a block without it `unverified`, and either blocks a final render (`functional.py render-gate`; handoff-v1 F6.3, N2). The approved version is frozen; changes after approval produce a new version and a new approval.
