...
1. **Locator resolvable by machine**, in one of six classes:
   ...
   - `<ficheiro>.docx|pdf|txt|md#¶n` (or `<file> · §<heading> ¶n` / `· ¶n` / `· p.n`) — an explicit passage of a document. A `.txt` numbers every non-empty line; a `.md` keeps its headings, so it carries the `§` form too. In a `.md`, `¶n` is the n-th non-empty line that is neither a heading nor a table row, counted from the top of the file — exactly the number `text_extract.py` writes as `[¶n]`; the `§<heading>` form names the section and keeps that same number (handoff-v1 F6.3, S6);
   ...
   **Decision record rows** (`handoff-v1` F2, maintainer decision 2026-09-23). A row whose id is `D-NNN` — the one `/decide`, `/frame` and `/blueprint` append so a decision is citable from the SU — points at an **authorised choice**, not at a fact. Its locator is its own record, `decisions.md#D-NNN`, and the block `## D-NNN` must exist in `decisions.md` (published in the same operation counts). Only `D-` ids take this path: a fact never becomes `Confirmed` by citing `decisions.md`.
...
3. **Human confirmation only from the process owner**, or from whoever the owner named as authority on the topic (recorded in `enquadramento.md`). A statement by anyone else — another team, IT, a sales rep — enters as an attributed declaration (transcript, document) or as `Assumed` with basis. The executor may confirm facts of the **engagement** itself — authorized scope, registration decisions — marked `[ÂMBITO AUTORIZADO]` in the claim, evidence = `decisions.md#D-NNN` of the **approval** that authorised it (a frame, solution or blueprint approval block, present; maintainer decision Q6, 2026-09-23). This mark is the only way a non-`D-` row may cite `decisions.md`: without it, citing the decisions file is not evidence. Resolving an `X-` or `R-` row "by internal evidence, without an owner answer" produces `Assumed was X-nnn` / `Assumed was R-nnn`, never `Confirmed`.
...
Two halves enforce this: the deterministic half (`dashboard.audit_confirmed_locators`, consulted by `pre-authority-guard.py` before a tool write and by `resolve.py publish` before a coordinator publication — both **refuse**, in a `handoff-v1` engagement, a new or changed `Confirmed` row without a resolvable locator — and the `sem locator` facet in `/status`) verifies that an anchor is there and that it opens, never that the claim is true, and an empty warning list means "every locator points at something that exists", never "the facts are right"; the judgement half lives in the lenses' Hard rules and in the round arbiter (`aisa-round` step 5c), whose finding is **never a metadata edit** — a `Confirmed` that says more than its evidence becomes `A-nnn was C-nnn`, append-only, original marked resolved.
