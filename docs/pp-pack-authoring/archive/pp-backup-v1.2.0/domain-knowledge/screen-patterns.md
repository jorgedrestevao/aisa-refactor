# screen-patterns — Screen Design Patterns (Canvas standard)

**Source:** v33/v34 `ref-10-screen-patterns.md` (direct port).
**Consumers:** `ux-cell`, `design-spec-cell`.
**Token tier:** WARM (loaded only by cells that declare it in `pack.yaml § domain_knowledge_load`).

**Scope:** Canvas standard components only. Rich/interactive prototyping decisions (layout density, animation, modal vs side-peek, etc.) belong to Claude Design (claude.ai/design) — out of scope here.
...
Vertical sidebar with home + 3–7 primary screens. Role-gated visibility per `security.matrices.screen_permissions`.
...
**Rule:** every task in `screens.user_tasks` MUST appear in at least one role's journey, OR be flagged as orphan.
...
**Rule:** every validation rule in `logic.validation_rules` MUST surface a user-facing message in `meta.language`.
...
**Transition note format:** `screens.transition_notes[].familiar_anchor` carries the Excel reference; `key_differences` documents what improves vs Excel.
