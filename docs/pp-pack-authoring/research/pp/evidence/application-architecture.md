...
Canonical version: **2026-09-02**, consolidated from `application-architecture-research-v2.md` after gate PASS (`application-architecture-gate.md`). Lineage: v1 draft (`application-architecture-v1.md`) → adversarial review (`application-architecture-review.md`) → v2 → gate. Cross-references `platform-suitability.md` (VALIDATED) as **PS-nn** / **U-nn** / **PS row n**.
Source policy: `../source-policy.md`.

**Purpose.** Answer: *"Given these application requirements, which application architecture should aisa consider — canvas, model-driven, converged (model-driven + custom pages), Power Pages (Liquid or SPA), code apps, Teams-hosted, embedded surfaces, Wrap, hybrid with Azure, or custom development — and when does the recommendation change?"* Not a product guide: capabilities appear only where they define a fit boundary.

**Provenance rule.** Sources are **fetched** (page retrieved, ms.date read from metadata — the default), **search-verified** (snippet only; a finding relying solely on such a source is capped at MEDIUM), or **T2/T3/T4** (non-Learn). Kind is recorded per source in §11.

**Origin tags.** Every finding carries **Origin:** MS (Microsoft statement) / INF (analyst inference from documented limits) / T3 (independent) / UNKNOWN. STRONG = "no documented constraint violated"; an INF-derived POOR is phrased "treat as unsupported until verified", never as a Microsoft statement.

**Overall confidence MEDIUM (gate).** Individual findings keep their own grade (HIGH where a fetched, dated Tier 1 statement supports them). The overall cap reflects: Pages-vs-custom boundary rests on T3 only; strict preview/GA banner state of Pages SPA sites and code apps unverified against client-rendered banners; maintainability ceiling has no Microsoft threshold; licence-dependent cells await Area 10; research, review and gate share an author.

**Relationship to platform-suitability.** Resolves R-14 (Wrap, Intune/MAM: AA-38..AA-41) and R-13 localisation (AA-48; push throttles stay UNKNOWN, AA-46). Updates PS-03 (code apps: GA wording, B2B guests supported, pipelines ALM; AA-35). Re-verifies PS-11, PS-35, PS-53; restores the PS-02 phone-browser condition (AA-45); refines PS row 3 / PS-16 at application level (AA-50). Softens PS-10 (custom UX no longer forces platform exit; it forces premium licensing and managed-host constraints — AA-34..AA-36) without overturning its brand-critical verdict.

**Volatility.** Modern controls, code apps, Pages SPA/code sites, Pages server logic, Entra External ID migration, model-driven "new look" are 2025–2026 features with short half-lives. AA-C3 (PCF licensing) and AA-C10 (code apps B2B) have already changed once. Re-verify before pack encoding (§9).
...
| 4 | **Power Pages (Liquid or SPA)** | Authenticated external users over Dataverse; portal-shaped interactions; WCAG/EN 301 549 mandates; multi-language (Liquid sites) | Anonymous/consumer audiences (capacity licensing + misconfiguration RISK); bespoke SPA UX (product-documented, no preview wording, but single-language, no Git, no PWA — strict GA state UNKNOWN); traffic spikes (U-4); near-real-time freshness | System of record not in Dataverse; offline data capture; third-party API surface (PS-36); server/caching control | MS + T3 | AA-21..AA-30 |
| 5 | **Code apps** | Bespoke SPA over connectors/Dataverse with platform governance; pro-dev team; internal or B2B audience; premium budget | Solutions + pipelines ALM (no Git integration); Conditional Access instead of IP restriction; strict GA banner state UNKNOWN | IP-allowlist estates (public asset endpoint); Windows player; offline (undocumented → treat unsupported); anonymous users | MS | AA-35 |
| 6 | **Teams-hosted app (Dataverse for Teams)** | Team-scoped app inside Teams; ≤ 2 GB / ~1M rows; seeded licences; no PCF, model-driven, offline, API, plug-ins needed | Growth beyond limits → one-way upgrade to full Dataverse (then premium licences for all users) | Any PS-16 exclusion; device hardware in Teams (camera/barcode/sensors unsupported) | MS | AA-50 |
...
Gate condition: rows 4 and 5 stay CONDITIONAL until the preview/GA banner state of V-09 and V-18 is confirmed in a browser (§9).

---

## 2. Decision boundaries (requirement → constraint → architectural implication)
...
- Projected screens beyond what one maker can hold, or ≥ 2–3 divergent persona experiences × canvas → partition (multiple apps + `Launch()` with state loss) or model-driven shell + custom pages; retrofit is manual (MS, AA-03). No Microsoft numeric threshold exists (U-A1); the 30–40-screen figure is T3/T4 heuristic and must not be encoded (AA-C1).
...
- Contact-centre multisession × custom model-driven app → unmanaged solution only; conflicts with managed-solution production doctrine (PS-05) → first-party app or governance exception (MS, AA-20).
...
- Long lifetime × low tolerance to vendor UI change → model-driven carries highest wave-driven UI exposure, canvas medium, code apps lowest UI exposure but full own-maintenance (MS + INF, AA-54).
...
- Audience identity tier → employees: any internal app type · known partners: B2B guests into canvas/model-driven/code apps/Wrap, conditional on cross-tenant licence recognition (per-app plans fail) · authenticated customers: Pages · anonymous public: Pages with AA-25 conditions, or custom (MS, AA-52).

---

## 3. Findings

Ids AA-nn are stable. Sub-registers in §11: A- (canvas), B- (model-driven), C- (Pages), D- (components/code apps), E- (mobile/offline/a11y/l10n), V- (verification fetches).
...
- **Evidence:** Partition guidance positions task-scoped apps as the healthy unit (A-03); comparison table: canvas "Full control" of UI, "Dataverse + many others using connectors", "Only responsive if designed in this way" vs model-driven "Limited, predominantly customization", "Dataverse only", "Automatically responsive" (B-01). No Microsoft numeric limit on screens or controls exists (U-A1). The vendor statement "100 tables and over 50 screens" (A-07) is inadmissible as capability evidence per PS-06 and is recorded only in AA-C1.
- **Decision impact:** Fit row 1. POOR verdicts come from audience, embedding (PS-12), data volume (PS-13) and organisational factors, not from a hard cap.
- **Confidence:** HIGH (facts) / MEDIUM (envelope).
- **Sources:** A-03, B-01.

#### AA-02 — Responsiveness is opt-in effort, not default; tooling has documented gaps
- **Classification:** CONSTRAINT + TRADE-OFF
- **Origin:** MS
- **Evidence:** "your entire layout scales to fit the screen… can't take advantage of the additional pixels"; "You activate responsiveness by turning off the app's Scale to fit setting, which is on by default"; "you adjust some settings and write expressions throughout your app" (A-02). "The authoring canvas doesn't respond to the sizing formulas created. To test responsive behavior, save and publish your app"; dragging a control "overwrite[s] those expressions or formulas"; Data table/Charts/Add picture "not supported in the layout containers"; Center/End options "can cause child controls inaccessible" (A-01, A-02).
- **Decision impact:** Multi-form-factor requirement raises canvas effort per screen with a publish-to-test loop.
- **Confidence:** HIGH.
...
- **Origin:** MS
- **Evidence:** "large apps can be split into smaller sections"; "State in the original app is lost when another app is launched" (A-03). Converged route: "Don't exceed 25 custom pages in a model-driven app"; a custom page "is expected to typically be a single screen with loose coupling"; standalone canvas apps "aren't supported for use as a custom page" (B-02).
- **Decision impact:** Plan partition seams up front; retrofitting is manual either way. The maintainability ceiling is not quantified by Microsoft (AA-C1).
- **Confidence:** HIGH.
...
- **Decision impact:** Long-lived apps need formula-length review on `.pa.yaml` source (PS-48) and named-formula refactoring.
- **Confidence:** HIGH.
- **Sources:** A-03.
...
- **Origin:** MS
- **Evidence:** "Using the OnStart property can cause performance problems"; `Navigate` in OnStart "is retired"; OnStart "might be disabled by default" (A-04). UDFs documented without preview flag: typed parameters/returns, behavioural bodies, user-defined types; "Recursion isn't yet supported"; named formulas cannot have side effects (A-04). Studio-load improvement ("as much as 80%", A-03) is a vendor measurement, cited as direction only.
- **Decision impact:** UDF-first + App.Formulas + StartScreen as build standard; heavy OnStart in an estate = refactoring backlog. Narrows, does not close, the "no subroutines" critique (no unit-test framework, AA-08).
...
- **Origin:** MS
- **Evidence:** "Git version control for editing apps has been removed and is no longer supported" (A-06, 2025-05-14). "Changes aren't available to commit until you publish"; "You can't edit the .pa.yaml files directly in your repository if your app contains code components" (A-05). "one maker can edit one custom page at a time" (B-02).
- **Decision impact:** Maker team > 1–2 per app is itself a partition / model-driven / code-apps signal. Stale co-authoring articles: AA-C2.
- **Confidence:** HIGH.
...
- **Origin:** MS
- **Evidence (V-07, 2026-05-22 upd. 2026-08-14):** "Effective April 2026, Test Engine is deprecated. The documentation and GitHub repository are no longer maintained by Microsoft"; "near-zero usage and failed to meet customer needs … The Power Fx implementation created unnecessary limitations that are avoided if using Playwright directly"; "The Power Platform Playwright samples guide the use of Playwright for Power Platform test automation."
- **Decision impact:** Long life + high change velocity → budget Playwright or accept manual regression; an argument for code apps/custom at high change velocity.
- **Confidence:** HIGH.
...
- **Origin:** MS (mechanism; security anchor via PS-31) + INF
- **Evidence:** Entry-branching example `If(LookUp(Attendees, User = User().Email).Staff, StaffPortal, HomeScreen)` (A-04). Implicitly shared connections expose the maker's credentials to all app users regardless of UI (PS-31/S-43) → access is governed by connection identity and data-source security, not by `Visible`. No single Learn sentence "hiding controls is not security" located (U-A5).
...
- **Sources:** A-04, PS-31, A-19 signals.
...
- **Classification:** DECISION CRITERION
- **Origin:** MS
- **Evidence:** "especially well suited to process driven apps that are data dense and make it easy for users to move between related records"; relationship navigation "Automatic, provided relationships exist" vs canvas "Only where designed and applied using Power Fx formulas"; "without a data model housed within Microsoft Dataverse, you can't create a model-driven app" (B-01).
...
- **Origin:** MS
- **Evidence:** "the controls of the default tab are always rendered when opening a record"; quick views, subgrids, timeline "produce the most strain"; "greatly slow form loading"; "Field-level security does have an impact on your app's performance. Limit its use" (B-08, B-07). Multisession (V-08, 2025-10-16): "Make sure that the system requirements for Copilot Service workspace are met"; **"You can enable the multisession experience in a model driven app within an unmanaged solution only"**; "This feature isn't supported in model driven apps installed by default, such as Sales Hub." Exact licence clause sits behind the linked requirements page (U-B1).
- **Why it matters:** Unmanaged-only conflicts with the enterprise doctrine "only managed solutions in production" (PS-05).
- **Decision impact:** Data-dense fit assumes tabbed forms + subgrid discipline; multisession requirement → weigh the Dynamics 365 Customer Service first-party app (rung 0) vs a custom app with a governance exception.
- **Confidence:** HIGH.
...
- **Decision impact:** Fit row 4; driver question: data in Dataverse + portal-shaped interaction? No Tier 1 decision tree (U-C2).
...
- **Origin:** MS
- **Evidence:** "Power Pages supports Bootstrap version 3.3.6 and Bootstrap version 5" (C-05). SPA product doc (V-09, 2026-07-21): "An SPA site is a Power Pages site that runs entirely in the user's browser (client-side rendering) … you manage SPA sites only through source code and command-line interface (CLI) tools." Limitations: "Power Platform Git integration isn't supported for Single-Page Application (SPA) websites"; Pages/Style workspaces and Liquid not supported; "Single-language support"; no OOB lists/forms; no Power Fx; SEO "limited"; "SPA sites don't support the progressive web app (PWA) setting"; no built-in testing; site version 9.7.4.x+, PAC CLI 1.44.x+. No preview qualifier in body text; strict GA banner status UNKNOWN (U-C6). Release plan GA 2026-01-31 (C-06) — supporting only. Licensing effects UNKNOWN (U-C1).
- **Why it matters:** The SPA route surrenders what makes Pages cheap (OOB forms/lists, Liquid, multi-language, PWA, Git); it is closer to "custom SPA hosted by Pages" than to "Power Pages with custom UI".
- **Decision impact:** External bespoke UX → SPA-on-Pages CONDITIONAL on: single language acceptable, no PWA/offline-read, pro-dev team, CLI-only ALM. Otherwise custom web compares directly (AA-30). Liquid sites remain the low-code path.
...
#### AA-25 — RISK: table-permission misconfiguration exposed millions of records (AppOmni 2024); Microsoft controls: anonymous-access indicator, daily security checks, deep scan; open registration ON by default
- **Classification:** RISK
- **Origin:** T3 (incident) + MS (controls, defaults)
- **Evidence:** AppOmni: "several million records" exposed; NHS supplier "over 1.1 million NHS employees'" PII; causes: Web API field wildcards, excessive Anonymous/Authenticated role permissions, Global access, no column security (C-07; C-15, C-16). Microsoft (V-10, 2025-03-12 upd. 2026-08-31): "Anonymous access to Dataverse tables shows the number of websites where anonymous access is allowed … at least one table permission that allows anonymous users to access the data"; "Security checks are run against all websites in your tenant, once every day automatically"; Deep scan "detecting and addressing vulnerabilities"; site health statuses Standard/Enhanced/Advanced. Table-permission snippet (search-verified): "If you add the Anonymous users web role to a table permission, the data in the table is visible to anyone". Default `OpenRegistrationEnabled: True` (C-03). Web API wildcard verbatim not fetched (U-C7).
- **Decision impact:** Mandatory acceptance conditions for anonymous/external designs: column security on PII tables; no Global read for Anonymous/Authenticated roles on sensitive tables; explicit Web API field allow-lists; open registration disabled unless required; PPAC security checks at "Advanced" in run-book.
- **Confidence:** HIGH.
...
#### AA-28 — CDN + WAF included; caching for anonymous users only; throughput ceilings unpublished (U-4)
- **Classification:** FACT + CONSTRAINT
- **Origin:** MS
...
- **Decision impact:** Lock components or accept drift; parameter-driven APIs as mitigation (U-D4).
- **Confidence:** HIGH.
...
- **Decision impact:** Visual PCF does not make a standard-connector app premium (AA-C3). PCF-in-canvas is UI extensibility only; custom data/auth patterns → code apps or custom. Requires long-term pro-dev ownership.
- **Confidence:** HIGH.
- **Sources:** D-04 (2025-07-01), D-05, D-06 (2026-08-20), D-07, D-12, D-14.

#### AA-35 — Code apps: GA wording, B2B guests supported at canvas parity, pipelines ALM; public asset endpoint and no Git integration persist (updates PS-03)
- **Classification:** FACT + CONSTRAINT
- **Origin:** MS (+ T2 GA blog)
- **Evidence:** GA blog 2026-02-05 (D-20). Overview (V-18, 2026-08-12 upd. 2026-08-19): no preview wording; "End users that run code apps need a Power Apps Premium license"; "Azure B2B (external user access) | End-users can share code apps and access them by using Azure B2B to access resources in a tenant, similar to canvas apps" (changed vs preview docs, AA-C10); "compiled app assets are served from a publicly accessible endpoint that doesn't support IP-based restrictions today. To restrict access by IP, use Conditional Access"; "Don't store sensitive user or organizational data in the app"; no Windows player; no PowerBIIntegration (embeddable via Power Apps visual); no SharePoint forms. ALM: `pa app push` into a solution; pipelines Dev→Test→Prod; "Don't support source code integration" (D-22). Mobile-player support and offline undocumented (U-D2, U-D3). Strict GA banner state UNKNOWN (U-C6). Licensing Guide treatment deferred (§7).
- **Decision impact:** Supported rung for internal AND B2B bespoke SPA UX; POOR for IP-allowlist estates, Windows-player audiences, offline, anonymous.
- **Confidence:** HIGH facts / MEDIUM GA-strict.
...
|---|---|---|---|
| 0. Configure / buy / first-party | An existing Dynamics 365 app, M365 feature or ISV product covers the requirement (e.g. multisession contact centre → Customer Service) | Licence for the first-party app; feature-fit gaps | PS-41, PS-44, AA-20 |
...
#### AA-48 — Localisation by app type: canvas manual; model-driven built-in; Pages Liquid 43 languages, Pages SPA single-language; RTL supported on custom pages, undocumented for standalone canvas
- **Classification:** DECISION CRITERION (resolves R-13 localisation)
- **Origin:** MS + UNKNOWN (canvas RTL)
- **Evidence:** Canvas: translation-table + `LookUp` on `Language()` pattern (E-23, 2021-01); locale formatting built in (E-24). Model-driven: "Dataverse enables you to install multiple language packs"; export→Excel→import (E-26, search-verified). Pages Liquid: 43 OOB languages + custom (E-25); Pages SPA: "Single-language support" (V-09). RTL (V-05, 2022-05 upd. 2025-05, page marked pre-release): "Right-To-Left (RTL) is fully supported for languages like Arabic and Hebrew and the page will change at runtime automatically"; exception "icons, shapes, and images". No Learn statement on RTL for standalone canvas apps exists either way (AA-C11, U-E6).
- **Decision impact:** > 2–3 languages → model-driven or Pages Liquid; canvas cost = screens × languages; SPA-on-Pages excluded for multi-language. RTL → custom pages documented; standalone canvas **UNKNOWN — treat as unsupported until verified by prototype**.
...
- **Evidence:** Power BI visual (V-13, 2025-03-04): "The maximum number of records that can be passed from Power BI to Power Apps visual using 'PowerBIIntegration' object is limited to 1000"; "Power Apps visual is only supported for Embed for your organization. Embed for your customers is not supported"; "The Power Apps visual can't filter the data or send any data back to the report"; write-back "Changes are reflected on the next scheduled refresh"; "You'll need to share the app separately from your report"; "Power BI Report Server doesn't support the Power Apps visual"; Safari view-only, Firefox unsupported. SharePoint customized forms (V-14, 2025-11-14; V-15, 2026-01-13): "You can't manually share a SharePoint form customized with Power Apps. Instead, any user who has at least Read or Restricted View access to the linked SharePoint list inherits access to the form"; "there's currently no automated method in Power Apps to copy a form from one environment to another"; generic lists/libraries only. Licensing statement not on fetched pages (U-S1); no Microsoft "when to go standalone" statement (INF).
- **Decision impact:** Power BI visual: STRONG for internal drill-through/write-back on ≤ 1,000 selected rows with refresh-latency tolerance; POOR for external report consumers, Report Server, immediate reflection. SharePoint form: STRONG for a form over one list where readers = list readers and no cross-environment ALM; INF: portability, solution management, multi-source logic or offline → standalone canvas app.
- **Confidence:** HIGH facts / MEDIUM "when standalone".
...
- **Classification:** DECISION CRITERION
- **Origin:** MS
- **Evidence:** Internal app types require Entra work/school accounts; personal Microsoft accounts removed (PS-60). B2B guests into canvas with cross-tenant-recognised licences — "Power Apps per app plans … can't be recognized across tenants"; guests need "a standalone browser session" (PS-36). Code apps: "End-users can share code apps and access them by using Azure B2B … similar to canvas apps" (V-18). Wrap supports B2B guests with explicit tenant setup (E-02, E-03). Model-driven: same Entra/B2B model via Dataverse security roles (PS-31). Consumer/anonymous/social/external-IdP identities: Power Pages only (AA-22). Frontline/shared-device identity UNKNOWN (U-14).
- **Decision impact:** Audience tier → architecture set (see §2 last boundary).
- **Confidence:** HIGH.
- **Sources:** PS-60, PS-36, PS-31, V-18, E-02, E-03, AA-22.

#### AA-53 — Application-level performance and concurrency: no documented concurrent-user ceiling for any app type; capacity is governed by data-source and request limits
- **Classification:** CONSTRAINT + UNKNOWN
- **Origin:** MS + UNKNOWN
- **Evidence:** No concurrent-user ceiling for canvas or model-driven; capacity governed by data-source and request limits (PS-20); service-protection limits are per user per 5 minutes and "interactive users rarely hit them" (PS-18); Pages limits page silent on RPS/concurrency (C-17, U-4). Design-level guidance: canvas — reduce data calls, delegation, avoid OnStart/collections (A-07, A-11); model-driven — default-tab rendering, subgrid/quick-view strain, FLS cost (B-08, B-07); Pages — cache and CDN only for anonymous (AA-27, AA-28). Load benchmarks: none published (U-2, deferred to Area 09).
- **Decision impact:** Concurrency does not differentiate app types by documentation; it differentiates by data path (Dataverse direct vs connectors vs Pages cache). Large-audience design → load test; keep PS-18/PS-19 request budgets in the estimate.
...
- **Evidence:** Mandatory semi-annual release waves and rolling deprecations (PS-50). Model-driven: "classic theming isn't honored" (B-05); new look mandatory (T3 D-19, U-B4); multisession unmanaged-only (V-08). Canvas: modern-control property renames Feb 2026 (AA-11); `Navigate` in OnStart retired, mobile deep links break May 2026 (A-04, A-14); Test Engine deprecated (V-07); co-authoring removed (A-06). Custom pages: double-publish coupling (B-03). Code apps: own framework upgrades; no Git integration (V-18, D-22). Pages: SPA no Git; enhanced data model migration; B2C sunset (V-06). ALM friction from B-01: model-driven "Simple" migration vs canvas "Potentially complex".
- **Decision impact:** Long lifetime + low tolerance to unrequested UI change → weigh model-driven's wave exposure against its ALM simplicity; code apps trade wave exposure for owning the frontend stack. Every option needs a change-absorption owner (PS-50, AP-16).
...
| AP-32 | Growing a Dataverse for Teams app toward the 2 GB / feature caps without budgeting the one-way upgrade | Converts all users to premium; no downgrade | MS | AA-50 |
...
|---|---|---|---|---|
| AA-C1 | Canvas scale ceiling | T1 vendor statement "50+ screens … excellent performance" (A-07) — inadmissible per PS-06 / source policy §7 | T3/T4 30–40-screen maintainability heuristic | Neither admissible as a threshold; no numeric cap exists (U-A1). Performance (with correct patterns) and maintainability (team/change-rate) are different axes; size seams by team and personas |
...
| AA-C5 | Pages CIAM recommendation | Learn recommends Azure AD B2C (C-03, 2026-02) | B2C closed 2025-05; P2 discontinued 2026-03 (V-06) | New builds → Entra External ID |
...
| AA-C10 | Code apps B2B guests | Earlier preview docs: unsupported | V-18 (2026-08): supported "similar to canvas apps" | Changed with GA |
| AA-C11 | RTL on standalone canvas | Earlier reading: "not available in standalone canvas apps" | V-05 fetched: clause absent; no Learn statement found | UNKNOWN; earlier claim withdrawn |
...
| U-B1 | Exact licence clause for multisession in custom apps | Contact-centre requirements | Fetch Copilot Service workspace requirements |
...
| U-C6 | Strict GA banner state of SPA sites and code apps (client-rendered banner risk) | Fit rows 4, 5 | Manual browser check of V-09, V-18 |
...
| U-T1 | Component-library reuse inside Teams-hosted canvas apps | Teams estate reuse | Compare page silent |
| U-S1 | Licensing of SharePoint customized forms | Low-end cost path | Licensing Guide (Area 10) |
| U-4 | Power Pages traffic/throughput ceilings | Public high-traffic sites | CONFIRMED STILL OPEN 2026-09 |
...
| Load benchmarks, concurrency behaviour, gateway sizing (U-2) | 09 | AA-53 stays "not documented — load test" |
...
Undelivered deferrals revert the corresponding cells to UNKNOWN, as in platform-suitability §7.

---

## 8. Evidence-quality notes

- **Fetched (default):** all decision-weight sources, including the V-01..V-19 verification set. **Search-verified residue:** B-09, B-14, C-14, C-19, C-21, E-14, E-15, E-17, E-21, E-22, E-26 — none carries a finding above MEDIUM alone.
- **Aged pages (> 18 months) still load-bearing:** A-01, A-03, A-10, D-03, E-04, E-06, E-11, E-19, E-20, E-23, E-24, V-02 (2022 upd. 2025), V-05 (2022, pre-release banner), V-16 (2020 upd. 2024). Findings on them carry MEDIUM on currency where noted.
- **Inadmissible statements:** A-07's "100 tables / 50 screens" — recorded only in AA-C1. Connector-count and "build faster" marketing figures not used.
...
- **Client-rendered banner risk:** V-09 and V-18 show no preview wording in fetched body text; strict banner state marked UNKNOWN (U-C6), not asserted.
...
- **Authorship:** research, review and gate were produced by the same author in one session; independent spot-check of V-05, V-08, V-09, V-18, V-19 recommended before pack authoring.

---

## 9. Gate conditions carried into downstream use (mandatory)

From `application-architecture-gate.md`:

1. Every verdict derived from §1/§2 keeps its origin tag (MS / INF / T3 / UNKNOWN); INF-based POOR verdicts (RTL on canvas, custom offline sync, lifecycle ranking) are phrased "treat as unsupported until verified", never as Microsoft statements.
2. Before pack authoring: confirm in a browser the preview/GA banner state of V-09 (Pages SPA) and V-18 (code apps); fetch the Pages Web API / table-permission page for the wildcard wording (U-C7); read the September 2026 Licensing Guide for code apps, custom pages under per-app plans, Wrap and SPA entitlements (§7).
...
5. Dimensions deferred in §7 back-reference this file; undelivered deferrals revert the corresponding cells to UNKNOWN.
6. Re-verify short-half-life facts (modern controls, code apps, SPA sites, server logic, Entra External ID, new look) before encoding; treat AA-C3 and AA-C10 as facts that have already changed once.
...
Kind: fetched / search-verified / cited (T2) / signals (T3/T4).
...
### 11.2 Model-driven + custom pages (B-)
...
### 11.4 Components, theming, code apps (D-)
...
### 11.5 Mobile, Wrap, offline, a11y, l10n (E-)
...
- Authenticated external users over Dataverse with portal-shaped interactions → Power Pages (Liquid).
- Structured backbone + few bespoke screens → model-driven shell + custom pages.
- Team-scoped app on seeded licences within Dataverse for Teams limits → Teams-hosted.
- Requirement already covered by a first-party/M365/ISV product → configure or buy.
...
- Anonymous/consumer audiences → Pages, conditional on capacity budget + hardened table permissions + PPAC checks.
- Brand-critical UX → ladder: modern controls → PCF → code apps (premium/user) before leaving the platform.
- Bespoke external UX → SPA-on-Pages only if single-language, no PWA, pro-dev team, CLI-only ALM.
...
- Multisession on a custom model-driven app → unmanaged solution only; weigh first-party app.

**Poor fit signals**
- Offline in browser/Teams/custom pages; offline writes on Pages; offline + FLS or flows.
- Branded app + push; B2C native mobile; sovereign-cloud Wrap.
- Model-driven via phone browser; pixel branding inside model-driven core pages.
...
- Power BI embed-for-customers / Report Server; portable SharePoint customized forms.

**Important decision criteria**
- Persona-experience count; screen count × change velocity; maker team size; form-factor mix and browser-vs-app expectation; device hardware; offline depth/surface; audience identity tier; branding depth; push requirement; deep-link frequency; accessibility regime; language count + RTL; UI-reuse scope; freshness expectations; pro-dev capacity; premium-licence budget; team vs enterprise scope; tolerance to vendor UI change; first-party coverage.

**Important anti-patterns**
- God canvas app; OnStart-heavy initialization; collect-everything delegation workarounds; copy-paste component reuse; library forks via "allow customization"; PCF-before-platform-controls; unreviewed third-party PCF; model-driven for device-capability field capture; Pages with anonymous/broad permissions and Web API wildcards; Wrap for consumer/push; Visible-formula "security"; BPF as logic engine; model-driven via phone browser; Dataverse for Teams growth without upgrade budget; SPA-on-Pages for multi-language/PWA; SharePoint form as portable app; custom multisession when first-party fits.

**Important unknowns**
- Pages throughput ceilings (U-4); canvas numeric caps (U-A1); strict GA banner state of SPA sites and code apps (U-C6); code apps mobile/offline; push throttles; kiosk mode; multisession licence clause; RTL on standalone canvas; per-app-type accessibility conformance; licensing cells pending Area 10.
