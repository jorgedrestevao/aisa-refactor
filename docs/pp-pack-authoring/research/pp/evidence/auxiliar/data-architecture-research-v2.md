Research Status: DRAFT v2 (targeted revision after adversarial review; five HIGH gaps and the flagged MEDIUM gaps closed; gate NOT yet run)
Research Confidence: MEDIUM-HIGH (up from MEDIUM in v1)
Gate: PENDING

# Data Architecture — Research Evidence (v2)

Research area: **03 — Data Architecture** (`../research-areas.md`).
Version: **2026-09-02, v2**. v1 was consolidated from five parallel sub-researches (`DV`, `SQ`, `SP`, `EX`, `XC`). v1 was reviewed adversarially (`data-architecture-review.md`) and judged **NEEDS MORE RESEARCH** on 5 HIGH-severity gaps and several MEDIUM gaps. This v2 closes those gaps with five additional targeted sub-researches: virtual tables at volume + Fabric consumption (`VT`), Dataverse→SQL replication paths + reconciliation (`SY`), Azure SQL store-choice evidence (`SQ2`), business-volume→platform-budget + concurrency + consistency (`SC`), and data quality/ownership vocabulary (`DQ`). v1's 61 `DA-nn` findings, the store-fit matrix, decision boundaries, anti-patterns and alternatives are carried forward **with corrections** where the review or the new research contradicted them; new findings are added under their own prefixes (`VT-nn`, `SY-nn`, `SQ2-nn`, `SC-nn`, `DQ-nn`) the same way v1 already cross-references `PS-nn` (`platform-suitability.md`) and `AA-nn` (`application-architecture.md`).
...
**Overall confidence MEDIUM-HIGH.** Raised from v1's MEDIUM because: the five HIGH-severity gaps are closed with 90 new Tier-1-dominant findings (all but a handful of the ~95 new sources are Microsoft Learn, fetched and dated); the two most consequential v1 gaps (no sourced Dataverse→SQL path; unsourced SQL matrix cells) are now resolved with hard evidence, including one explicit negative finding in each case. Still capped below HIGH because: no adversarial review or gate has run on v2 itself; a handful of new unknowns remain genuinely open (virtual-table throughput at volume — `VT-09` — is a documented absence, not a measured answer; whether business rules enforce on Web API writes is a genuine unresolved contradiction in Microsoft's own docs — `DQ-10`); and several MEDIUM gaps from the review (archiving/retention IG coverage, full D365 IG sweep) are now closed but on AI-assisted, 3-year-refresh-cycle Microsoft content (`DQ-01`).
...
Raised from v1's MEDIUM. Justification:
...
Held below HIGH because:

- **No adversarial review or gate has run on v2 itself.** Every correction above is self-assessed by the same research lineage that produced v1; independent verification of the corrections (not just of the original claims) has not occurred.
- **One genuinely unresolved contradiction remains inside Microsoft's own documentation** (`DQ-10`, business-rule server-side enforcement) that cannot be closed by further reading — it requires an empirical test this research pass could not perform.
...
- **A handful of new unknowns are irreducibly open** (no published concurrent-user ceiling, no published standard-table row ceiling, no published write-amplification factor) not because research was insufficient, but because Microsoft does not publish the answer — these are correctly represented as engagement-measured inputs, not as gaps to be closed by more reading.

**Recommendation:** proceed to gate review. The remaining open items (see "Gaps Still Open") are bounded, well-defined, and do not block a defensible architecture decision — they bound the *precision* of specific numeric claims, not the *direction* of the guidance.
