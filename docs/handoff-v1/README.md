...
| Fase corrente | **F8**, em `main` (todo o programa F0–F8.2 já lá): desenho decidido (Q1–Q5), pilotos preparados (F8.1) e **primeiro piloto completo (execução R3, fx-02) integrado** — custo real medido e avaliação `pass` (F8.2). **Próximo: autorização do mantenedor para as restantes seis execuções (Q5)** — custo e achados em [F8/RELATORIO.md](F8/RELATORIO.md) §7 |
