...
| F2.4 Retoma a frio | integrado | (este) | `workflow.py resume` só lê: perfil → bootstrap com o checkpoint no read-set → checkpoint → reconciliação proposta → frescura recalculada → contexto com orçamento (ids de todos os críticos sempre) → próxima acção com razão. `/status` 2b e `/resume` consultam-no. `test_handoff_resume.py` 9 casos (T09 num processo novo, T16, D02, recuperação, legado, objectivo por declarar). Full 86/86, 2793; stdlib 67/67, 1995; ambos exit 0 |
