...
2. Aplica 09_DEPENDENCIAS_E_TESTES.md: não herdes dependências por antiguidade. Em F0 justifica consumidores e compatibilidade necessários; não mantenhas runtime classic completo sem essa necessidade. Não migres engagements existentes sem autorização específica.
...
9. Classifica os grupos de testes em manter/adaptar/consolidar/isolar/eliminar. Os 46 cenários são cobertura, não adições obrigatórias. Atualiza testes e código juntos; executa testes afetados no desenvolvimento, testes da fase no fecho e suite completa aplicável antes de integração/release. Não exijas verde em testes de comportamentos retirados nem removas testes só porque falham. Regista pass/fail/not-run com comando e evidência. Testes simulados não são pilotos humanos.
