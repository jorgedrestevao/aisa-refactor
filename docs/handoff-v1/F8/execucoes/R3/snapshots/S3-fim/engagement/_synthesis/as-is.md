...
5. Excepção: pedidos urgentes (avaria que impede trabalhar) vão directamente a compras; a chefia valida depois, no mesmo dia (C-003). Não está declarado se esta via também salta a validação da direcção financeira quando o valor ultrapassa o limiar — conflito aberto, X-001.
...
| via de urgência: encomendar antes, validar depois no mesmo dia | compras, chefia | mesmo dia (validação) | Confirmed — C-003 (o único tempo declarado nesta lente) | entrevista-processo.md · ¶5 |
...
O ponto único de conhecimento tribal identificado é o valor do despacho financeiro — ninguém sabia de cor antes da resposta do dono (resolvido → C-006). A via de urgência (C-003) é a excepção documentada mais relevante, e é também a origem do único conflito ainda aberto no engagement (X-001): salta a aprovação da chefia por desenho, mas não está escrito se também salta o controlo financeiro.
