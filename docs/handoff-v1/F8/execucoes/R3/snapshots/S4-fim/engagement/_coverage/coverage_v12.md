...
| | |
|---|---|
| registo | `_coverage/coverage_v12.json` |
...
| `COV-UNREVIEWED` | error | PM-U-007 | obrigação que uma revisão anterior deste passo (_coverage/coverage_v11.json) tratava e esta deixou cair: PM-U-007 — uma obrigação não desaparece, recebe uma disposição; repetir a revisão que a largou não a apaga (§4.4.4) | tratar a obrigação, nem que seja com `disposition: retire` e a autoridade que o permite |
