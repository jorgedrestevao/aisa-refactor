# Process Model — f8-r8-fx03

> Sources: pedido.md (a7da9323), entrevista-processo.md (8a709386), matriz-papeis.md (be7ebaa5), nota-dados.md (990dc582), nota-urgentes.md (44e9eab9), mudancas.md (eba5ccb7)  |  Generated: 2026-09-24T16:30:00Z  |  Capture run: 3
> Source dispositions: pedido.md USED · entrevista-processo.md USED · matriz-papeis.md USED · nota-dados.md USED · nota-urgentes.md USED · mudancas.md USED
> Nota de identidade: runs 1-2 deste ficheiro traziam o cabeçalho `f8-r3-fx02` por lapso (mesmo caminho, `projects/f8-r8-fx03/_capture/`) — corrigido nesta run (3), conteúdo inalterado por esta correcção.
...
- `mudancas.md` — três mudanças de negócio **pós-aprovação do handoff**, entregues pelo cliente nesta reabertura (R8): [X1] regra de arredondamento muda (linha a linha, meio-para-par); [X2] correcção de gralha na descrição da jornada ("encomeda"→"encomenda"), sem mudança de significado — não gera linha de síntese própria; [X3] novo despacho baixa o limiar da direcção financeira de 5 000 € para 3 000 €. USED — todo o ficheiro é material (3 parágrafos); é a fonte mais recente do engagement e, nos pontos [X1]/[X3], prevalece sobre o que estava `Confirmed` (C-007, C-006) antes desta reabertura.
...
- UNKNOWN — ponto exacto do arredondamento ao cêntimo (linha a linha ou só no total) — já causou diferenças de um cêntimo com a contabilidade — ver PM-U-001 (entrevista-processo.md ¶3). Entretanto respondido e registado como `Confirmed` (C-007: só ao total, meio-para-cima) antes desta reabertura — ver bullet seguinte para a mudança pós-aprovação.
- OBSERVED — pós-aprovação do handoff, a contabilidade decide mudar a regra: o arredondamento ao cêntimo passa a ser aplicado **linha a linha** (já não só ao total do pedido), pela regra **meio-para-par** (já não meio-para-cima), para alinhar com a facturação do fornecedor — muda o que estava `Confirmed` (C-007) no registo antes desta reabertura; consequência de dados: cada linha do pedido passa a precisar do seu próprio valor arredondado, não só o total (mudancas.md ¶1 [X1]).
...
- UNKNOWN — valor exacto do limiar que faz a direcção financeira decidir (aprovar) — ver PM-U-002 (entrevista-processo.md ¶4). Entretanto respondido e registado como `Confirmed` (C-006: 5 000 €, despacho) antes desta reabertura — ver bullet seguinte para a mudança pós-aprovação.
- OBSERVED — pós-aprovação do handoff, novo despacho baixa o limiar que obriga a ir à direcção financeira (M-2) de **5 000 € para 3 000 €** — muda o valor que estava `Confirmed` (C-006) no registo antes desta reabertura (mudancas.md ¶3 [X3]).
- UNKNOWN — nota-urgentes.md não diz se o caminho de urgência, com o novo prazo alargado, também passa a saltar (ou não) a aprovação da direcção financeira quando o pedido urgente ultrapassa o limiar do despacho — o conflito já registado (X-001 na Shared Understanding) mantém-se aberto, sem resposta nesta nota; `mudancas.md` também não o resolve — só muda o valor do limiar sobre que o conflito incide (X-001 mantém-se aberto, agora sobre 3 000 €).
...
- OBSERVED — `mudancas.md` (esta reabertura) muda dois factos já `Confirmed` antes da aprovação do handoff: o ponto de arredondamento (C-007 → linha a linha, meio-para-par) e o valor do limiar financeiro (C-006 → 3 000 €); nenhum dos dois é uma pergunta em aberto — são decisões de negócio já tomadas, a avaliar quanto ao impacto na arquitectura autorizada (`_blueprint/ux-blueprint_v01.yaml`) e na decisão registada (D-002) em `/options --reopen`, não a reabrir aqui.
...
Nada foi excluído por protecção, macro ou formato ilegível — todas as seis fontes de texto extraíram com `status: ok` (inclui `mudancas.md`, esta reabertura). Não há fonte estruturada (`.xlsx`/`.xlsm`) neste engagement, logo não há VBA, ligações externas, folhas protegidas ou fórmulas não replayable a listar. O artefacto "despacho" original que definia o limiar de aprovação financeira (enquadramento.md, passo 4d2) nunca foi entregue como ficheiro — o valor chegou por resposta do dono (`Confirmed` C-006, 5 000 €) e agora `mudancas.md` [X3] declara um despacho novo que o altera para 3 000 €; nenhum dos dois despachos em si (o original nem o novo) foi entregue como documento — o valor continua a vir por declaração, não por ficheiro lido.
