...
Estado depois do F7.4: F7.1–F7.4 integrados e com push; full 111/111 e stdlib 92/92 verdes.

Próxima acção segura: **F7.5** — escrever o gate no relatório (§6: T35 não aplicável por decisão, opção A; T36 = F1 `test_handoff_legacy`/`test_handoff_workflow` + `test_f7_compat`; T41 = `test_impact` T41 + `test_impact_gates`; T42 = `test_impact` T42; «rollback preserva informação nova e bloqueia downgrade destrutivo» = `test_f7_compat` D10 + rollback), rever limitações, marcar o estado *gate avaliado — aguarda aceitação*, actualizar `docs/handoff-v1/README.md` e pedir a aceitação ao mantenedor por `AskUserQuestion`.
