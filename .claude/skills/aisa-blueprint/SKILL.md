...
13c. **Functional contracts — the behaviour the render may never invent** (`handoff-v1` F4; `library/kernel/handoff-contract.md` → *Functional contract*; `docs/handoff-v1/F4/DESENHO.md`). Skip in draft mode. For the authorized scope of this version, write inline the functional contracts (`FC-NNNN`) the deliverables will need: what each journey (`J-NNNN`) does, rule by rule — inputs by `field_ref` (`<record_authority.key>.<campo>` of this version, never a field redefined), rule or transition, postconditions, exceptions (duplication, repetition, cancellation, concurrency, recovery where they apply), and positive, negative and boundary acceptance examples, with units and rounding where there is a calculation (`calculation`). A scope with no human surface owes no screen; its contracts still owe trigger, repetition, idempotency and recovery. A field that does not apply goes to `not_applicable` with its reason; no number is invented; a parameter still to choose is a `delegated_choices` entry with envelope, owner and acceptance, or it stays a blocker.
