...
1. **Pre-flight — the architecture entry gate.** Resolve the engagement; read `_state.json`.
   ...
   **One or more bounded scopes architectable** ⇒ architect **only** those scopes.
   ...
   Doing the review: `coverage.py inventory --engagement <slug> --json` gives the **denominator** — every unit of `context.json`, `enquadramento.md`, `answers.md`, the SU, the current solution decision, `inputs/`, `_capture/` and the phase artefacts. The motor builds it from the sources, never from what you write, so omitting a unit and its review line together does not produce a green. Write one `source_review[]` entry per unit or declared group (`reviewed` · `unverifiable` · `not_applicable`, each with its rationale) and one `coverage[]` entry per **material obligation**, with its disposition, the requirement it serves and what must happen to it. Then the two passes of §9 (described in step 13b). **With a process map** (`_map/map.json`), the denominator also carries every step, hand-off, detail and question of the map, each `CALC-NNN` qualified by its workbook and each material §4 label (`coverage-contract.md` §6.1.1). Read the map first — `python library/kernel/tools/process_map.py summary --engagement <slug> --task blueprint --json` (every block with its detail and evidence, journey by journey), `project --engagement <slug> --json` (the rows already about each element) and `render` for the view — together with the decisions taken since, so the AS-IS is read through what was decided and never re-imposed: for each material element state whether its need is **preserved**, **changed** (with the decision), **excluded** (with the scope authority) or **pending**, and put its unit in `source_unit_refs` of that obligation's item — an element read as `material` or `undetermined` with no item is `COV-MAP-UNPLACED`. One requirement served by two elements is one item with both units, never two items (the estimate counts it once). A new TO-BE need the owner declared enters with an empty `source_unit_refs` and its SU row — never with an invented origin in the files. Draft to a temp file **outside the engagement**, then publish:
...
2. **Author the `architecture:` block** (the contract). Facts serialized by `/decide` are read, not re-decided; `authorization` and `architectability_basis` come from step 1; everything else is architecture work.
...
10. **Fragment iteration.** Perform the iteration from recorded data:
...
11. **Carry the open questions and structural choices.** Any design decision blocked by an Unknown goes to `open_questions` (never a silent default). If a NEW unknown emerges, append the `U-NNN` row to the SU first — through a draft (`resolve.py draft … --files shared-understanding.md` → edit the copy → `resolve.py publish`; `library/kernel/orchestration.md` → *Writing an authority*) —, then reference it. Record architecture-level uncertainty in `open_architecture_choices[]`.
...
13b. **Coverage of the version just written — the second question, and never the same as the first.** Step 13 says the file is well formed. It says nothing about whether the design carries what the sources asked for: the case this mechanism exists for is a version the structural check calls `valid: yes (0 block, 0 warn)` that dropped a requirement the SU already carried. Two passes, both required for `semantic_review.status: completed` (`coverage-contract.md` §9):
    ...
    - **Source → target.** For each material obligation — request, invariants, clarifications, outputs **and their consumers**, the normal path, exceptions, permissions and segregation, data and calculations, integrations, failure and recovery, proofs — state the treatment and the **concrete point of this design** that carries it. With a process map, walk it journey by journey: the obligation inherited from a map element is carried by a functional destination of the kind the element needs (the object or data it produces, the action, the calculation, the automation, the integration, the access, the exception handling, the output and who receives it) — never by a node that only names the subject. An entity that mentions the subject is not a publication path, and a valid `su_ref` on a screen demonstrates nothing about whether the output is produced and consumed.
...
13c. **Functional contracts — the behaviour the render may never invent** (`library/kernel/handoff-contract.md` → *Functional contract*; `docs/handoff-v1/F4/DESENHO.md`). Skip in draft mode. For the authorized scope of this version, write inline the functional contracts (`FC-NNNN`) the deliverables will need: what each journey (`J-NNNN`) does, rule by rule — inputs by `field_ref` (`<record_authority.key>.<campo>` of this version, never a field redefined), rule or transition, postconditions, exceptions (duplication, repetition, cancellation, concurrency, recovery where they apply), and positive, negative and boundary acceptance examples, with units and rounding where there is a calculation (`calculation`). A scope with no human surface owes no screen; its contracts still owe trigger, repetition, idempotency and recovery. A field that does not apply goes to `not_applicable` with its reason; no number is invented; a parameter still to choose is a `delegated_choices` entry with envelope, owner and acceptance, or it stays a blocker.

    ```
    python library/kernel/tools/functional.py draft --engagement <slug>           # copy in _drafts/FCDRAFT-…/
    python library/kernel/tools/functional.py check --engagement <slug> --draft <id>
    python library/kernel/tools/functional.py publish --engagement <slug> --draft <id>
    ```

    `based_on` names this blueprint version with its `sha256`. `publish` refuses on integrity (`INTEGRITY_FAILURE`: a dead reference, a reused or silently dropped id, a revision that does not move by one) and on a stale base (`STALE_INPUT`: open a new draft). A contract with gaps is published with `BLOCKING_GAP` and is not authorisable — never fill the gap to make it pass.

    **Revalidation, never a hash swap** (`library/kernel/handoff-contract.md` → *Dependencies*). A different `sha256` proves the design changed; it does not say whether the change is editorial. When something the contracts rest on moved since the previous revision — the design file (same path, other content) or the basis of a cited row — `publish` refuses with `REVALIDATION_REQUIRED` and names what moved, and every render and release reads the contracts as `BASIS_CHANGED` until then. Assess each contract that depends on it against the change and add `revalidation` to the draft: `items` (per `FC`, `still_valid` when its content did not change, `updated` when you changed it), `assessment` (what changed and why each contract holds, or how it changed) and `assessed_by` (your role). The motor records it, with from/to and the date, in `revalidations`, which only grow. Updating the sha alone revalidates nothing; an `updated` contract goes back to the owner (step 16), a `still_valid` one keeps its authorisation.

    **Contradiction with the design** (T21). `publish` and `functional.py show` report every `FC_BLUEPRINT_CONFLICT`: the design and a contract declare different `required`, `values`, `default` or `type` for the same field. Never pick a side. Append one `Conflicted` row per conflict in the SU (`partes = desenho∧contrato funcional`, both locators, `quem decide: role: <owner_role>`), through a draft and `resolve.py publish`; the conflict stands until the design changes, the contract changes, or the owner decides.

    **Independent review — one subagent, after publication** (`CLAUDE.md` → *Delegação a subagentes*). Launch **one** subagent with the Agent tool, `subagent_type: fc-reviewer`, fresh context, passing only paths: the engagement root, `_design/functional-contracts.json` (the revision just published), this blueprint version, `shared-understanding.md` and `decisions.md`. It returns findings (target, severity, premise/evidence, failure scenario, closing condition) and the coverage of its review. Save them verbatim to `_blueprint/fc-review-r<NNNN>.md`. Give each finding a disposition (*Review policy*): accepted and corrected in a new revision (back to `draft`), rejected with evidence, delegated with an envelope, escalated to the owner (`AskUserQuestion`), or deferred with its impact. A review of an earlier revision does not cover this one.
...
14. **Output** to the user:
   ...
   Contratos do comportamento: <N> escritos (revisão <NNNN>) · <N> com lacunas que impedem a autorização · <N> em contradição com o desenho · revistos por um revisor independente: <sim | não> · autorizados pelo dono: <N> <| ainda nenhum>.
...
15. **On approval** (user says the business approved v<NN>): run **three** mechanical checks, in this order, and let none speak for another.
    ...
    **(iii) Functional coherence** — `python library/kernel/tools/functional.py conflicts --engagement <slug> --blueprint _blueprint/ux-blueprint_v<NN>.yaml`. Exit 4 → this version contradicts a current functional contract on a field: approval is refused until reconciled (T21; the `Conflicted` row of step 13c says what to settle).
    ...
    **Then, and only then, ask.** The approval request goes through `AskUserQuestion` (CLAUDE.md, inviolable rule), with the version, what it commits to, and the open questions that remain visible in the options. A question in prose at the end of a reply is not a question asked, and silence is not an approval.
    ...
    On a clear yes: generate the block with the motor — `python library/kernel/tools/functional.py approval-block --engagement <slug> --version <NN> --validated-by "owner (<role>, … via AskUserQuestion)"`, which carries the version's `**Blueprint sha256**` (T43 N2: a later change to that file makes the approval `stale` and blocks every final render; a block without it reads `unverified`) and refuses a date earlier than a recorded decision — and append that exact `D-NNN — Blueprint bp-v<NN> aprovado` block to `decisions.md` + the matching SU row (`| D-NNN | … | decisions.md#D-NNN | <today> | organizacional | <round> |`, the decision-record rule of `library/kernel/states.md` → *Confirmed threshold*) — **one** draft, `resolve.py draft --engagement <slug> --files decisions.md shared-understanding.md --reads _blueprint/ux-blueprint_v<NN>.yaml _coverage/coverage_vNN.json --json`, then `resolve.py publish` (`library/kernel/orchestration.md` → *Writing an authority*; a blueprint or coverage record that changed since it was shown is `STALE_INPUT`) —, and note it in `blueprint-log.md` **with the coverage record the approval rests on** (`_coverage/coverage_vNN.json`) — the reference points from the approval to the review, never the other way round, so the approval cannot invalidate the review it consumes. Later renders read this version. Approvals already on file are never rewritten, superseded or revoked by this rule; where an older version was approved before the review existed, that approval stands and its coverage simply reads as not verified.

15b. **Scope of the delivery** (`library/kernel/handoff-contract.md` → *Work packages and completeness*; `docs/handoff-v1/F6/DESENHO.md` §0 and Q3). After the approval of this version and before authorising contracts — step 16 names this scope. Skip in draft mode. One `SCOPE-NNNN` per delivery: `includes` — the SU rows and the journeys (`J-NNNN` of existing contracts) this delivery commits to, each with its reason; `excludes` — what the owner leaves out of this delivery, each with its reason and the `authorization_ref` of the decision that excludes it; `authorized_by` — that decision. The scope follows the decision's authorised scope and the request, and never widens them. Nothing is excluded on the executor's initiative: an item with an open blocker is either included (and blocks, visibly) or proposed to the owner for exclusion, with what the exclusion drops — its promises, its dependencies, its effort (plan 02 §8). Ask the owner through `AskUserQuestion` (business language: what is delivered, what is left out and why); a prose question does not count and silence is not a yes. On a clear yes, append one block to `decisions.md` with its decision-record row in the SU, in **one** draft (`resolve.py draft --engagement <slug> --files decisions.md shared-understanding.md --json`, then `resolve.py publish`):

    ```
    ## D-NNN — Âmbito de entrega SCOPE-NNNN

    - **Inclui**: <itens, com os ids>
    - **Exclui**: <item (id) — motivo> · … | nada
    - **Validated by**: owner (<papel>, via AskUserQuestion)
    - **Timestamp**: <ISO-8601>
    ```

    Only then write the scope, through the motor — the only publisher (`_design/` is guarded) — with `authorized_by` and each exclusion's `authorization_ref` pointing to that decision (a draft opened before the decision reads a base that moved: open it after):

    ```
    python library/kernel/tools/inventory.py draft --engagement <slug> --kind scope        # copy in _drafts/SCOPEDRAFT-…/
    python library/kernel/tools/inventory.py check --engagement <slug> --kind scope --draft <id>
    python library/kernel/tools/inventory.py publish --engagement <slug> --kind scope --draft <id>
    ```

    `publish` refuses an exclusion whose decision does not exist, a reference that does not resolve and a revision that does not move by one. The owner declines → the scope is published without `authorized_by`: a visible gap that keeps every release `preliminary`, never a scope authorised on the owner's behalf.

16. **Authorising functional contracts** (the owner, never an agent — plano 05 F4 item 6). Only contracts that `functional.py show` reports `authorizable` (no gap, no conflict) and that the `fc-reviewer` read in this revision. Ask through `AskUserQuestion` (business language: what each contract commits to, the assumed premises it rests on — `assumed_premises` —, what stays delegated); a prose question does not count and silence is not a yes. On a clear yes, generate the block with the motor — `python library/kernel/tools/functional.py authorization-block --engagement <slug> --fc FC-NNNN [--fc …] --validated-by "owner (<papel>, via AskUserQuestion)" --scope <SCOPE>` (the `SCOPE-NNNN` of step 15b) — never by hand: it computes each fingerprint and refuses a contract with gaps or a validator that is not a human role. Append it to `decisions.md` with its decision-record row in the SU, in **one** draft (`resolve.py draft --engagement <slug> --files decisions.md shared-understanding.md --reads _design/functional-contracts.json --json`, then `resolve.py publish`). A contract that changes afterwards reads `stale` and needs a new authorisation; the others keep theirs. Authorising a TO-BE contract confirms no AS-IS premise: the `Assumed` rows stay `Assumed`.

17. **Work inventory** (`library/kernel/handoff-contract.md` → *Work packages and completeness*, *Traceability*). After the authorisations of step 16. Skip in draft mode. The implementation-spec owns the inventory and `/render` renders it from this file (it may not invent work); the estimate is the only owner of effort. One `WP-NNNN` per unit of build, configuration, migration, proof or acceptance of the included scope: `scope_id`; `purpose`; `realizes` — the `FC-NNNN`, SU rows, decisions or blueprint nodes it serves (a node as `<file>#<selector>` that resolves to exactly one node); `proves` — the blueprint proof obligations it discharges; the WPs it depends on; `acceptance` — conditions that use the contracts' examples (`fc`); `skills`; `definition_of_done`; `not_applicable` with its reason. **No effort field**: the motor refuses it.

    ```
    python library/kernel/tools/inventory.py draft --engagement <slug> --kind work-packages   # copy in _drafts/WPDRAFT-…/
    python library/kernel/tools/inventory.py check --engagement <slug> --kind work-packages --draft <id>
    python library/kernel/tools/inventory.py publish --engagement <slug> --kind work-packages --draft <id>
    ```

    `publish` pins, for each WP, the fingerprint of every contract it realises or tests against and the `sha256` of every design file whose nodes it realises or proves; a later change to either reads `CONTRACT_CHANGED`/`BASIS_CHANGED` and blocks the final version. Republishing over a moved pin needs the same `revalidation` as step 13c (per WP, `still_valid` or `updated`, the assessment, your role) — `REVALIDATION_REQUIRED` otherwise. A new inventory revision makes the rendered implementation-spec and estimate stale (they cite `inventário r<N>`): `/render` again.

    Then `python library/kernel/tools/trace.py show --engagement <slug>` and `trace.py scope-gate --engagement <slug>`. Each finding goes back to its owner — a missing contract to step 13c, missing work or acceptance here, an open viability proof to the owner — or stays visible: never a WP invented to silence a finding, never an exclusion without the owner (step 15b). A WP that realises nothing and says not why is a finding, not a shortcut.
...
11. **Behaviour lives in the functional contracts, not in the design.** The design references `FC` ids (`fc_refs`) and never restates a rule; a rule written into the design is a finding for the `fc-reviewer`. The contracts are published only by `functional.py` (`_design/` is guarded), authorised only by the owner, and an authorisation covers only the fingerprints it lists.
