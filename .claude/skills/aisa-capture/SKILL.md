...
1. **Resolve the engagement root** (as in `aisa-start`): `$AISA_ENGAGEMENTS_ROOT/<slug>` if set, else `projects/<slug>`. Read `_state.json` for the slug. The capture-run counter is **derived**, never stored: the highest `run N` of an `L2` line in `_capture/_capture-log.md` (none → 0; the motor reports it as `engagement.capture_run`).
...
5. **L2 — process model** (single LLM pass, only after ALL structured files did L1+L3, all text files did LT **and** the evidence index was rebuilt from those results — run steps 5b and 6 before this step; the index a2 inspects must be the current one, never the previous run's):
   a0. **Enquadramento before interpretation (P-0).** Read `enquadramento.md` **before** reading any capture output to interpret it: T1 actors, T2 trigger, T3 activities, T4 outcomes (and their consumers), T5 invariants (`M-n`), T6 failure today, T7 change requested. It is the owner's declared hypothesis of how the process works — it **orients** what to look for and which outputs, actors and exceptions must be found; it is **never a filter**. Evidence the enquadramento does not mention, or that contradicts it, stays in the model with its marker and locator — never dropped for not fitting. Record the outcome in the model header (§1): `P-0: consumed (T1–T7)` · `P-0: absent — <no enquadramento.md | "não declarado">` (then the L2 runs on the files alone and says so; nothing is inferred in its place). Every **divergence** between what the owner declared and what the sources show — an output the files produce that T4 does not name, a T4 outcome no source produces, an actor or step that differs — is one §4 line citing **both sides** (`enquadramento.md#T<n>` and the source locator), marked by the evidence it rests on; a material divergence also gets its §6 `PM-U` (who settles it: the owner). Consuming the P-0 changes no SU row and confirms no `M-n` — `/frame` does that.
   ...
   c. This pass is run N+1 (N from step 1): stamp it in the model header and in the step-7 `L2` line. `_state.json` is **not** written — this skill writes no authority (`library/kernel/orchestration.md` → *Writing an authority*).
...
6. **Evidence index** (always, even when nothing was captured). Runs **after** steps 3–5b and **before** step 5 — the L2 pass reads it (step 5a2), so it is rebuilt from the current extractions first; the L2 pass does not change it, so it is not rebuilt again afterwards:
   ```
...
6. **Raw files stay authoritative**: on any conflict between the model and the raw file, the raw file wins and the analysis records a Conflicted SU row (`aisa-round`; `library/kernel/lens-checklists.md`).
...
11. **The enquadramento orients, never filters.** The L2 pass reads `enquadramento.md` first (step 5a0); what the owner did not declare, or declared differently, stays visible as a divergence line — it is never discarded for not fitting, and the enquadramento is never edited to match the files. An absent P-0 is stated, never reconstructed.
