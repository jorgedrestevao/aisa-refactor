...
5b. **O engagement nasce com grafo — antes da primeira autoridade.** Logo depois de a pasta existir, e antes de escrever `_state.json`, a SU, `decisions.md`, `answers.md` ou as linhas `M-n` (passos 7–9b), correr uma vez:
    ...
    Publica um grafo vazio e válido pelo coordenador (recibo em `_ops/receipts/graph-init.json`). Idempotente: correr outra vez devolve `already`, e um grafo já existente nunca é substituído. **Não é opcional**, e a ordem também não: ausência de grafo bloqueia. Com `_state.json` escrito e sem grafo, o guarda de autoridade (`pre-authority-guard.py`) recusa escrever a SU e as decisões; com linhas na SU, `init` recusa (`NOT_EMPTY`) — isso migra-se. Depois do passo 7 já não há ordem que funcione. Tudo o que os passos 6–9c escrevem vai para **um** rascunho (passo 5c) e é publicado numa só operação do coordenador (passo 9d) — autoridades, linhas `R-00` e o seu espelho no grafo, com um recibo; um Edit directo da SU não é espelhado por hook. Falha aqui → parar e reportar o erro tal como veio; não continuar para o passo 6 com o engagement por nascer.
